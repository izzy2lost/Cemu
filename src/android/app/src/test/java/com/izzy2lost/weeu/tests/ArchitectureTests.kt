package com.izzy2lost.weeu.tests
...
class ArchitectureTests {
    ...
    private val isFromGeneratedCode =
        object : DescribedPredicate<JavaClass>("is from generated code") {
            override fun test(input: JavaClass?): Boolean {
                ...
                return input?.packageName == "com.izzy2lost.weeu"
                        || input?.packageName?.contains("databinding") ?: false
            }
        }

    private val isFromTests = resideInAPackage("com.izzy2lost.weeu.tests..")

    private val isFromNativeInterface = resideInAPackage("com.izzy2lost.weeu.nativeinterface..")
    ...
    fun `feature packages should not depend on each other`(javaClasses: JavaClasses) {
        SlicesRuleDefinition.slices().matching("com.izzy2lost.weeu.(*)..")
            .should().notDependOnEachOther()
            .ignoreDependency(
                alwaysTrue(),
                or(
                    resideInAPackage("com.izzy2lost.weeu.common.."),
                    resideInAPackage("com.izzy2lost.weeu.nativeinterface.."),
                    resideInAPackage("com.izzy2lost.weeu.databinding..")
                )
            )
    ...
    fun `native interface should not depend on any other app packages`(javaClasses: JavaClasses) {
        noClasses().that(isFromNativeInterface)
            .should().dependOnClassesThat(
                resideInAPackage("com.izzy2lost.weeu..")
